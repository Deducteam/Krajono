...
inductive max_spec (n:nat) (f:nat→bool) : nat→Prop ≝
 | found_max_spec : ∀m:nat.m < n → f m =true →
 (∀i. m < i → i < n → f i = false) → max_spec n f m
 | not_found_max_spec : (∀i.i < n → f i = false) → max_spec n f O.
...
inductive min_spec (n,b:nat) (f:nat→bool) : nat→Prop ≝
 | found_min_spec : ∀m:nat. b ≤ m → m < n + b → f m =true →
 (∀i. b ≤ i → i < m → f i = false) → min_spec n b f m
 | not_found_min_spec : (∀i.b ≤ i → i < (n + b) → f i = false) → min_spec n b f (n+b).
