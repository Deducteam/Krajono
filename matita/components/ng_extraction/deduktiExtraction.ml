...
let translate_const (baseuri, name) =
  try Hashtbl.find reference_table (baseuri, name) with
  | Not_found ->
...
let fresh_univ name =
  ...
  let avoid constname' = Hashtbl.mem constraints_table (modname', constname') in
  let constname' = fresh avoid (escape_name name) in
  (modname', constname')

let translate_univ_uri (baseuri, name) =
  try Hashtbl.find universe_table name with
  | Not_found ->
...
module UMSet = Set.Make(struct type t = D.constname * univ let compare = compare end)
...
let sorted_universes () =
  ...
  let rec insert univ =
    if List.mem univ !sorted then
      (* Universe has already been added *)
      ()
...
module Translation (I : INFO) =
  struct
    let translate_reference reference =
      let R.Ref (uri, spec) = reference in
      let baseuri = U.baseuri_of_uri uri in
      let name = NCicPp.r2s I.status true reference in
      translate_const (baseuri, name)

    let lift i term =
      NCicSubstitution.lift I.status i term

    let subst term1 term2 =
      NCicSubstitution.subst I.status term1 term2
    ...
    let whd context term =
      NCicReduction.whd I.status ~subst:[] context.cic term
    ...
    let get_inductive_type uri =
      let (is_inductive, leftno, inductives, _, i) =
    ...
    let rec translate_term context term =
      ...
      | C.LetIn (x, a, n, m) ->
        ...
        let () = add_entry (fst c') (D.Definition (snd c', Some lifted_a', lifted_n')) in
      ...
      | C.Match (R.Ref (uri, R.Ind (_, _, leftno)) as ind_ref, return_type, matched, cases) ->
        ...
        let return_sort =
          match snd (split_prods context (-1) (type_of context return_type)) with
          | C.Sort s -> s
          | _ -> failwith "invalid return type"
        in
        let match_const' = translate_match_const (ind_baseuri, ind_name) return_sort in
        ...
        D.apps (D.Const match_const') (
          left_ind_args' @ [return_type'] @ cases' @ right_ind_args' @ [matched'])
    ...
    and translate_args context terms ty =
      ...
      | n :: ns ->
        let a, b =
          match whd context ty with
          | C.Prod (x, a, b) -> a, b
          | _ -> failwith "Left term of an application should have product type"
        in
    ...
    let rec translate_bindings context (bindings : (string * C.term) list) translated : context * ((D.var * D.term) list) =
      match bindings with
      | (x, a) :: bindings ->
    ...
    let translate_match_scheme leftno ind_name arity constructors sort =
      ...
      let return_type_type' =
        to_cic_prods (List.rev (right_ind_params' @ [quant_var_name', quant_var_type'])) (
          univ_type sort')
      in
      let context = {
          context with
      ...
      let translate_case (cons_name, right_cons_params, right_cons_args) =
        ...
        let case_name' = fresh_var context.dk (Format.sprintf "case_%s" cons_name) in
        ...
        let case_type' =
          to_cic_prods (List.rev right_cons_params') (
            term_type sort' (
...
(*      let context = {
    ...
    let translate_filter_scheme leftno ind_name arity constructors sort =
      ...
      in
      let return_type_type' =
        to_cic_prods [(quant_var_name', quant_var_type')] (univ_type (term_of_univ sort'))
      in
      let context = {
          context with
          dk = (return_type_name', return_type_type') :: context.dk
      ...
      add_entry (fst @@ filter_const' sort)
...
(*      Format.printf "new name: %s@." (snd fun_const'); *)
      ...
      let filter_ind' = translate_filter_const (U.baseuri_of_uri ind_uri , U.name_of_uri ind_uri) in
      ...
      in
      let fun_body_type'' = to_cic_prods (List.rev (params' @ [rec_param'])) return_type'' in
      let () =
      ...
      in
      let right_fun_term' =
        D.apps (D.Const (filter_ind' return_sort)) (ind_args' @ [
    ...
    let translate_obj_kind obj_kind =
      ...
      | C.Constant (_, name, None, ty, _) ->
        HLog.debug (Format.sprintf "Dedukti: Translating constant declaration %s" name);
      ...
      | C.Constant (_, name, Some(body), ty, _) ->
        HLog.debug (Format.sprintf "Dedukti: Translating constant definition %s" name);
        ...
        in
        if List.mem name problematic then translate_declaration name ty else
        translate_definition name ty body
      ...
      | C.Inductive (is_inductive, leftno, types, _) ->
        HLog.debug (Format.sprintf "Dedukti: Translating inductive definitions");
...
let extract_obj status obj =
  if extraction_enabled () then
    let (uri, depth, metasenv, subst, obj_kind) = obj in
    ...
    HLog.message (Format.sprintf "Dedukti: Extracting object %s" (U.string_of_uri uri));
    (* There should be no unresolved meta-variables at this point. *)
    ...
    HLog.message (Format.sprintf "Dedukti: Done extracting object %s" (U.string_of_uri uri))

(** This function is called every time a constraint is added to the library. **)
let extract_constraint status u1 u2 =
  if extraction_enabled () then
...
let filename_of_modname modname =
  Format.sprintf "%s.dk" modname
